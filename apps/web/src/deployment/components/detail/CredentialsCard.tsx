...
/**
 * Carte d'accès affichée une seule fois au passage « running ». Le mot de passe
 * (d'EXEMPLE en display-only) est masqué par défaut, révélable, et accompagné
 * d'un avertissement « non récupérable ensuite ».
 */
  ...
  return (
    <Card className="p-4">
      <div className="mb-2 flex items-center gap-2">
        ...
        <span className="text-text-primary text-[12px] font-semibold">Accès (exemple)</span>
      </div>
      <dl className="space-y-1.5 font-mono text-[11.5px]">
        <div className="flex justify-between gap-2">
          <dt className="text-text-muted">host</dt>
          <dd className="text-text-primary break-all">{access.host}</dd>
        </div>
        <div className="flex justify-between gap-2">
          <dt className="text-text-muted">port</dt>
          <dd className="text-text-primary">{access.port}</dd>
        </div>
        <div className="flex justify-between gap-2">
          <dt className="text-text-muted">user</dt>
          <dd className="text-text-primary break-all">{access.user}</dd>
        </div>
      ...
      <p className="text-text-muted mt-3 flex items-start gap-1.5 text-[11px] leading-relaxed">
        ...
        Affiché une seule fois. Le mot de passe ne sera pas récupérable ensuite (exemple
        display-only).
      </p>
