"""Tests d'integration de la migration Stack contre un Postgres reel.

Couvre, contre un Postgres reel (testcontainers) :
- `upgrade head` cree les 3 tables stack (stacks, stack_services, stack_links)
  et laisse un head unique (pas de branche Alembic introduite par cette slice).
- un cycle `downgrade -1` (annule la migration stack) puis `upgrade head` est
...
# integration/ -> __tests__/ -> models/ -> infrastructure/ -> stack/ -> app/ -> apps/api
_API_ROOT = Path(__file__).resolve().parents[6]

_TABLES_STACK = {"stacks", "stack_services", "stack_links"}
_STACK_REVISION = "f1a2b3c4d5e6"
...
class TestStackMigrationHead:
    ...
    async def test_upgrade_head_cree_les_tables_stack(
        ...
        assert tables >= _TABLES_STACK


class TestStackMigrationDowngrade:
    async def test_cycle_downgrade_puis_upgrade_est_rejouable(
        ...
        command.upgrade(config, "head")
        # Annule uniquement la migration stack (revient sur l'etat precedent).
        command.downgrade(config, f"{_STACK_REVISION}-1")

        tables_apres_downgrade = await _existing_tables(url)
        assert _TABLES_STACK.isdisjoint(tables_apres_downgrade)

        # Rejouable : aucune erreur de type enum deja existant.
        command.upgrade(config, "head")
        tables_apres_reupgrade = await _existing_tables(url)
        assert tables_apres_reupgrade >= _TABLES_STACK
