"""Tests d'integration de la migration `env_var` (template_params) sur Postgres reel.

Couvre, contre un Postgres reel (testcontainers) :
- `upgrade head` ajoute la colonne `env_var` a `template_params` et laisse un head
  unique (pas de branche).
- un cycle `downgrade -1` (retire `env_var`) puis `upgrade head` est rejouable :
...
_ENV_VAR_REVISION = "a1b2c3d4e5f6"
_TABLE = "template_params"
_COLUMN = "env_var"
...
class TestEnvVarMigrationHead:
    def test_un_seul_head(self) -> None:
        script = ScriptDirectory(str(_API_ROOT / "alembic"))

        assert len(script.get_heads()) == 1

    async def test_upgrade_head_ajoute_la_colonne_env_var(
...
class TestEnvVarMigrationDowngrade:
    async def test_cycle_downgrade_puis_upgrade_est_rejouable(
        ...
        command.upgrade(config, "head")
        # Annule uniquement l'ajout de env_var (revient sur la migration precedente).
        command.downgrade(config, f"{_ENV_VAR_REVISION}-1")

        columns_apres_downgrade = await _columns(url, _TABLE)
        assert _COLUMN not in columns_apres_downgrade

        # Rejouable : aucune erreur (colonne deja existante / absente).
        command.upgrade(config, "head")
        columns_apres_reupgrade = await _columns(url, _TABLE)
        assert _COLUMN in columns_apres_reupgrade
