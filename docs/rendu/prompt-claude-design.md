...
11. **Qualité, méthodologie & gestion de projet** : TDD strict (Red→Green→Blue), **chiffres de tests réels** (~1 184 tests back / ~903 front, **~95 % de couverture**, 11 migrations), gates CI multi-lanes (lint/format/types/sécurité/tests/**couverture**) + **durcissement supply-chain** (Semgrep épinglé tag+digest, actions tierces pin SHA, `permissions:` least-privilege, gitleaks/Checkov/Trivy/SonarCloud) ; **gestion de projet** : pilotage **Kanban** sur board Jira **STN** (réconcilié sur la réalité du code), **versioning SemVer dérivé des conventional commits** (`feat`→minor / `fix`→patch), Trunk-Based Dev + **cycle de review** (rapport d'étonnement), **DoR/DoD**, **développement multi-agents en git worktrees** (stacks Docker isolées), et **organisation d'équipe** (Dev / Cyber / Design-QA).
...
13. **Bilan & roadmap versionnée** : ce que le MVP atteint + **roadmap par version SemVer alignée sur Jira** (tableau Version → item) — **v0.79** (cycle de vie par service, vraie pause `docker pause`, durcissement Checkov/SonarCloud, quotas, export CSV), **v0.80** (MFA TOTP, 2ᵉ LLM « juge », anti prompt-injection), **v1.0** (déploiements Terraform/Proxmox → débloque les 10 cartes provider).
14. **Indicateurs & métriques projet** : encadré/infographie des chiffres clés (version, templates, tests, couverture, migrations, slices, lanes CI, environnements).
15. **Rétrospective & leçons apprises** : ce qui a bien marché (TDD, dev multi-agents, mockups→prod, Clean Archi, anti-hallucination) et les difficultés/ajustements (synchronisation du backlog Jira, périmètre stacks bêta, CI réparée & durcie) — avec un « si c'était à refaire ».
16. **Glossaire & annexes** : glossaire des termes (IDP, IaC, SSE, RBAC, JWT, LLM, TDD, TBD, vertical slicing, gate de déployabilité) ; annexes (liens dépôt / Jira / roadmap, tableau des 4 environnements + déclencheurs de déploiement).

### Chiffres clés à faire ressortir (encadrés/infographies)
**Version v0.78.0** · 45 templates au catalogue · **31 déployables** / 14 bloqués (10 Terraform + 4 runtimes) · **~1 184 tests backend** + **~903 frontend** · **couverture ~95 %** · 11 migrations · 4 environnements · freemium **Free 0 € / Pro ~9 € / Team ~25 €/user / Entreprise sur devis / self-host gratuit**.
